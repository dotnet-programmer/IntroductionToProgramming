...
try
{
	...
	int numer;
	while ((!int.TryParse(Console.ReadLine(), out numer)) || (numer < 1 || numer > 10))
	{
...
static void Zadanie_2_2()
{
	...
	do
	{
		...
		wspolczynniki = Console.ReadLine()!.Split(' ');
	} while (!double.TryParse(wspolczynniki[0], out a) || !double.TryParse(wspolczynniki[1], out b) || !double.TryParse(wspolczynniki[2], out c));
...
static void Zadanie_2_3()
{
	...
	do
	{
		...
		wspolczynniki = Console.ReadLine()!.Split(' ');
	} while (!double.TryParse(wspolczynniki[0], out masa) || !double.TryParse(wspolczynniki[1], out wzrost));
	Console.WriteLine($"BMI = {masa / (wzrost * wzrost)}");
}

// Po wykonaniu programu: int x = 100; Console.WriteLine(++x * 2);
// a) wyświetli się liczba 202 b) wyświetli się liczba 200 c) będzie błąd d) wyświetli się liczba 201.
// Najpierw oblicz wyświetlaną wartość i wybierz jedną z odpowiedzi (a, b, c, d), a dopiero później sprawdź wynik uruchamiając program.
static void Zadanie_2_4()
{
...
Console.WriteLine("wynik={0} x={1} y={2} z={3}", wynik, x, y, z);
*/
static void Zadanie_2_9()
{
	Console.WriteLine("Zadanie 2.9");
	int x = 1, y = 4, z = 2;
	bool wynik = (x++ > 1 && y++ == 4 && z-- > 0);
	Console.WriteLine("a) wynik={0} x={1} y={2} z={3}", wynik, x, y, z); // false, x=2, y=4, z=2

	x = 1; y = 4; z = 2;
	wynik = (x++ > 1 & y++ == 4 && z-- > 0);
	Console.WriteLine("b) wynik={0} x={1} y={2} z={3}", wynik, x, y, z); // false, x=2, y=5, z=2

	x = 1; y = 4; z = 2;
	wynik = (x++ > 1 & y++ == 4 & z-- > 0);
	Console.WriteLine("c) wynik={0} x={1} y={2} z={3}", wynik, x, y, z); // false, x=2, y=5, z=1

	x = 1; y = 3; z = 4;
	wynik = (x == 1 || y++ > 2 || ++z > 0);
	Console.WriteLine("d) wynik={0} x={1} y={2} z={3}", wynik, x, y, z); // true, x=1, y=3, z=4

	x = 1; y = 3; z = 4;
	wynik = (x == 1 | y++ > 2 || ++z > 0);
	Console.WriteLine("e) wynik={0} x={1} y={2} z={3}", wynik, x, y, z); // true, x=1, y=4, z=4
